...
(async () => {
  ...
  try {
    ...
    for (const ratio of ['16:9', '4:3', '3:2', '1:1', '8:5', '4:5', '3:1']) {
      ...
      const originalHeight = Number(await page.getByLabel('高さ px').inputValue());
      await page.getByRole('button', { name: '縦横入れ替え', exact: true }).click();
      assert.equal(await page.getByLabel('比率', { exact: true }).inputValue(), `${h}:${w}`);
      assert.equal(Number(await page.getByLabel('幅 px').inputValue()), originalHeight);
      assert.equal(Number(await page.getByLabel('高さ px').inputValue()), 1200);
      assert.equal(await frame.evaluate(e => e.style.aspectRatio), `${originalHeight} / 1200`);
      const pending = page.waitForEvent('download');
      await page.getByRole('button', { name: '画像をダウンロード' }).click();
      const download = await pending;
      const png = fs.readFileSync(await download.path());
      assert.equal(png.readUInt32BE(16), originalHeight);
      assert.equal(png.readUInt32BE(20), 1200);
      await page.getByRole('button', { name: '縦横入れ替え', exact: true }).click();
      assert.equal(await page.getByLabel('比率', { exact: true }).inputValue(), ratio);
      assert.equal(Number(await page.getByLabel('幅 px').inputValue()), 1200);
      assert.equal(Number(await page.getByLabel('高さ px').inputValue()), originalHeight);
    }
    await page.getByLabel('比率', { exact: true }).selectOption('8:5');
    await page.getByLabel('幅 px').fill('800');
    await page.getByLabel('回転角度', { exact: true }).fill('15');
    await page.getByRole('button', { name: '左右反転', exact: true }).click();
    await page.getByRole('button', { name: '右へ1px' }).click();
    await page.getByRole('button', { name: '縦横入れ替え', exact: true }).click();
    assert.equal(await page.getByLabel('幅 px').inputValue(), '500');
    assert.equal(await page.getByLabel('高さ px').inputValue(), '800');
    assert.equal(await page.getByLabel('回転角度', { exact: true }).inputValue(), '0');
    assert.equal(await page.getByRole('button', { name: '左右反転', exact: true }).getAttribute('aria-pressed'), 'false');
    await page.getByText('X 0 / Y 0 px', { exact: true }).waitFor();
    assert.ok(Math.abs(Number(await zoom.inputValue()) - 800 / 360 * 100) < .1);
    await page.setViewportSize({ width: 390, height: 844 });
    await page.getByRole('button', { name: '縦横入れ替え', exact: true }).click();
    assert.equal(await page.getByLabel('比率', { exact: true }).inputValue(), '8:5');
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
    await page.setViewportSize({ width: 1440, height: 1100 });
    await page.getByLabel('幅 px').fill('');
    assert.equal(await page.getByRole('button', { name: '縦横入れ替え', exact: true }).isDisabled(), true);
    await page.getByLabel('サイズの指定方法').selectOption('template');
    assert.equal(await page.getByRole('button', { name: '縦横入れ替え', exact: true }).count(), 0);
    assert.equal(await frame.evaluate(e => e.style.aspectRatio), '1280 / 720');
